...
*/
	...
	function VPagination($query = "", $rows_to_show = "", $qstring = "") {
		$this->total_rows = mysqli_num_rows($this->dbLink, mysqli_query($this->dbLink, $query));
	...
	*/
		...
		$this->results = mysqli_query($this->dbLink, $query);
	}

	//creates Previous 1 2 3 4 Next
	function get_navigation() {
		echo "<div class='pagination'>"; //pagination css starts here
		$this->gen_prev_link();
	...
	function gen_prev_link() {
		...
		if($this->get_current_page() != 1) {
			echo $this->gen_href($this->get_index() - $this->get_rows_to_show(), "Prev") . " "; 
		}
	}
	...
	function getRows() {
		...
		while($row = mysqli_fetch_array($this->results, MYSQL_BOTH)) {
				$retVal[] = $row;
		}
