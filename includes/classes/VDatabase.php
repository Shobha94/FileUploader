...
class VDatabase {
	...
	function insertRow($sql) {
			mysqli_query($this->dbLink, $sql) or die('Insert Query Error: '.mysqli_error());
	}
	
	
	function updateRow($sql) {
		mysqli_query($this->dbLink, $sql) or die('Update Query Error: '.mysqli_error());
		return mysqli_affected_rows($this->dbLink);
	}
	
	
	function deleteRow($sql) {
		mysqli_query($this->dbLink, $sql) or die('Delete Query Error: '.mysqli_error());
		return mysqli_affected_rows($this->dbLink);
	}
	
	function noOfRows($sql) {
		$rs = mysqli_query($this->dbLink, $sql) or die('Query Error: '.mysqli_error());
	...
	function getAutoID() {
		$id = -1;
		$rs = mysqli_query($this->dbLink, "SELECT LAST_INSERT_ID()");
	...
	function getRow($query) {
		$row = null;
		$rs = mysqli_query($this->dbLink, $query) or die('Query Error: '.mysqli_error());
